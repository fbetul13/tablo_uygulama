INSERT INTO llm_platform.data_prepare_modules (user_id,asistan_id,query,working_platform,query_name,database_id,db_schema,documents_id,csv_database_id,csv_db_schema,data_prep_code) VALUES
...
','logo_veritabani','satis_siparis_verileri',2,'dbo',NULL,1,'llm_asistant_file_info',NULL),
...
','logo_veritabani','irsaliye_fatura_gecikmeleri',2,'dbo',NULL,1,'llm_asistant_file_info',NULL),
	 (1,2,'SELECT 
...
left join msystems_table m on r.companyid = m.companyid and r.msystemid = m.msystemid;','logo_veritabani','raw_logs',3,'raw',NULL,1,'llm_asistant_file_info',NULL),
	 (1,2,'Select * From [ManageMind].[dbo].[Recetesi_Satis_Siparisi_Olmayan_Alımlar];','logo_veritabani','recete_ve_siparisi_olmayan',2,'dbo',NULL,1,'llm_asistant_file_info',NULL),
	 (1,2,'SELECT * FROM [ManageMind].[dbo].[ARS_Girilmemiş_Faturalar_UrunTedarikcileri]
ORDER BY 1,6 DESC','logo_veritabani','Girilmemis_Faturalarin_Bildirilmesi',2,'dbo',NULL,1,'llm_asistant_file_info',NULL),
	 (1,1,'SELECT 
    TRIM(REGEXP_REPLACE(c.companyname, ''\s+'', '' '', ''g'')) AS companyname,
    TRIM(REGEXP_REPLACE(m.displayname, ''\s+'', '' '', ''g'')) AS displayname,
    a.requestid,
...
FROM
    ...
    INNER JOIN msystems msys ON msys.companyid = jd.companyid AND msys.msystemid = jd.msystemid;','2024-08-27 07:15:41.636','2025-05-14 10:44:53.880534','bulut_pc','mekatronik_sistem_tartımları',3,'consolide',NULL,1,'llm_asistant_file_info',NULL),
	 (1,2,'SELECT * FROM [ManageMind].[dbo].[ARS_Girilmemiş_Faturalar_MüşteriIadeleri]','logo_veritabani','erp_ye_girilmemiş_satis_iade',2,'dbo',NULL,1,'llm_asistant_file_info',NULL),
	 (1,1,'SELECT
